...
impl Styles {
    ...
    pub fn with_blue(blue: Color) -> Styles {
        let header = ColorSpec::new().set_fg(Some(Color::White)).clone();

        Styles {
            header_bug: header.clone().set_bg(Some(Color::Red)).clone(),
            header_error: header.clone().set_bg(Some(Color::Red)).clone(),
            header_warning: header.clone().set_bg(Some(Color::Yellow)).clone(),
            header_note: header.clone().set_bg(Some(Color::Green)).clone(),
